...
export class Skeleton {
    ...
    private isValidEdgeEvent(event: EdgeEvent): boolean {
        return !event.nodeA.processed && !event.nodeB.processed && 
    ...
    private pushNextSplitEvent(node: VertexNode): void {
        ...
        while (current && current !== node && !visited.has(current)) {
            visited.add(current);
    ...
    private registerEdgeEvent(nodeA: VertexNode, nodeB: VertexNode): void {
        ...
        if (intersection.distance < currentTime - this.timeEpsilon) return;
    ...
    public compute(): StraightSkeletonResult | null {
        ...
        VertexNode.resetIds();
        this._initialize();
        const lav = new LAV(this.polygon);
        ...
        while (this.queue!.length > 0 && iterations++ < iterationLimit) {
